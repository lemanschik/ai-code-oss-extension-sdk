...
function getProtobusServiceNames(packageDefinition) {
    ...
    const protobusServiceNames = Object.keys(packageDefinition).filter((name) => name.startsWith("cline.") || name.startsWith("grpc.health"));
...
function wrapStreamingResponseHandler(handler, controller) {
    return async (call) => {
        try {
            ...
            const responseHandler = (response, isLast, sequenceNumber) => {
                try {
