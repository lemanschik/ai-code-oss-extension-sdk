...
export class TerminalProcess extends EventEmitter {
    ...
    async emitCurrentTerminalContents() {
        try {
    ...
    async initializeForNewCommand(command) {
    ...
    async runWithShellIntegration(terminal, command) {
    ...
    async executeCommandWithShellIntegration(terminal, command) {
        try {
    ...
    setupNoOutputTimeout(command, streamState) {
    ...
    async processOutputStream(stream, command, streamState, firstChunkTimeout) {
    ...
    async processChunk(data, command, streamState) {
    ...
    removeCommandEcho(data, command, streamState) {
    ...
    detectCompilationStatus(data) {
        ...
        return (compilingMarkers.some((marker) => data.toLowerCase().includes(marker.toLowerCase())) &&
            !markerNullifiers.some((nullifier) => data.toLowerCase().includes(nullifier.toLowerCase())));
    }
    handleChunkOutput(data, streamState) {
    ...
    cleanupAfterStream(streamState, firstChunkTimeout) {
    ...
    async handleCommandCompletion(command, streamState) {
    ...
    async runWithoutShellIntegration(terminal, command) {
