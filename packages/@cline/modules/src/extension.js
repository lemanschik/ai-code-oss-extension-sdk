...
import { migrateCustomInstructionsToGlobalRules, migrateWelcomeViewCompleted, migrateWorkspaceToGlobalStorage, } from "./core/storage/state-migrations";
...
export async function activate(context) {
    ...
    let installId = context.globalState.get("installId");
    if (!installId) {
        installId = uuidv4();
        await context.globalState.update("installId", installId);
    }
    telemetryService.captureExtensionActivated(installId);
    context.subscriptions.push(vscode.commands.registerCommand("cline.plusButtonClicked", async (webview) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.mcpButtonClicked", (webview) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.settingsButtonClicked", (webview) => {
        const isSidebar = !webview;
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.historyButtonClicked", async (webview) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.accountButtonClicked", (webview) => {
    ...
    */
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.addToChat", async (range, diagnostics) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.addTerminalOutputToChat", async () => {
    ...
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider("*", new (class {
        ...
        provideCodeActions(document, range, context) {
            ...
            if (editor &&
                ...
                expandedRange = new vscode.Range(Math.max(0, range.start.line - CONTEXT_LINES_TO_EXPAND), START_OF_LINE_CHAR_INDEX, Math.min(document.lineCount - LINE_COUNT_ADJUSTMENT_FOR_ZERO_INDEXING, range.end.line + CONTEXT_LINES_TO_EXPAND), document.lineAt(Math.min(document.lineCount - LINE_COUNT_ADJUSTMENT_FOR_ZERO_INDEXING, range.end.line + CONTEXT_LINES_TO_EXPAND)).text.length);
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.fixWithCline", async (range, diagnostics) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.explainCode", async (range) => {
        await vscode.commands.executeCommand("cline.focusChatInput"); // Ensure Cline is visible and input focused
        await pWaitFor(() => !!WebviewProvider.getVisibleInstance());
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.improveCode", async (range) => {
        await vscode.commands.executeCommand("cline.focusChatInput"); // Ensure Cline is visible and input focused
        await pWaitFor(() => !!WebviewProvider.getVisibleInstance());
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.focusChatInput", async () => {
        let activeWebviewProvider = WebviewProvider.getVisibleInstance();
        ...
        if (activeWebviewProvider?.getWebview() && activeWebviewProvider.getWebview().hasOwnProperty("reveal")) {
            const panelView = activeWebviewProvider.getWebview();
            ...
            if (!activeWebviewProvider) {
                ...
                if (tabInstances.length > 0) {
                    ...
                    if (potentialTabInstance.getWebview() && potentialTabInstance.getWebview().hasOwnProperty("reveal")) {
                        const panelView = potentialTabInstance.getWebview();
                        panelView.reveal(panelView.viewColumn);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.openWalkthrough", async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cline.generateGitCommitMessage", async (scm) => {
    ...
    context.subscriptions.push(context.secrets.onDidChange(async (event) => {
...
function maybeSetupHostProviders(context) {
    if (!HostProvider.isInitialized()) {
        console.log("Setting up vscode host providers...");
        const createWebview = function (type) {
