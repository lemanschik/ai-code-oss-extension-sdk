...
export class Controller {
    ...
    accountService;
    cacheService;
    constructor(context, postMessage, id) {
    ...
    async getCurrentMode() {
        return (await getGlobalState(this.context, "mode")) || "act";
    }
    /*
    ...
    */
    ...
    async initTask(task, images, files, historyItem) {
        ...
        const apiConfiguration = this.cacheService.getApiConfiguration();
        const { autoApprovalSettings, browserSettings, preferredLanguage, openaiReasoningEffort, mode, shellIntegrationTimeout, terminalReuseEnabled, terminalOutputLineLimit, defaultTerminalProfile, enableCheckpointsSetting, isNewUser, taskHistory, strictPlanModeEnabled, } = await getAllExtensionState(this.context);
        ...
        this.task = new Task(this.context, this.mcpHub, this.workspaceTracker, (historyItem) => this.updateTaskHistory(historyItem), () => this.postStateToWebview(), (taskId) => this.reinitExistingTaskFromId(taskId), () => this.cancelTask(), apiConfiguration, autoApprovalSettings, browserSettings, preferredLanguage, openaiReasoningEffort, mode, strictPlanModeEnabled ?? false, shellIntegrationTimeout, terminalReuseEnabled ?? true, terminalOutputLineLimit ?? 500, defaultTerminalProfile ?? "default", enableCheckpointsSetting ?? true, await getCwd(getDesktopDir()), this.cacheService, task, images, files, historyItem);
    }
    ...
    async togglePlanActMode(modeToSwitchTo, chatContent) {
        ...
        if (this.task) {
            ...
            if (this.task.taskState.isAwaitingPlanResponse && didSwitchToActMode) {
                ...
                await this.task.handleWebviewAskResponse("messageResponse", chatContent?.message || "PLAN_MODE_TOGGLE_RESPONSE", chatContent?.images || [], chatContent?.files || []);
                return true;
            }
            else {
                this.cancelTask();
    ...
    async getFileMentionFromPath(filePath) {
    ...
    async getStateToPostToWebview() {
        // Get API configuration from cache for immediate access
        const apiConfiguration = this.cacheService.getApiConfiguration();
        const { lastShownAnnouncementId, taskHistory, autoApprovalSettings, browserSettings, preferredLanguage, openaiReasoningEffort, mode, strictPlanModeEnabled, userInfo, mcpMarketplaceEnabled, mcpDisplayMode, telemetrySetting, planActSeparateModelsSetting, enableCheckpointsSetting, globalClineRulesToggles, globalWorkflowToggles, shellIntegrationTimeout, terminalReuseEnabled, defaultTerminalProfile, isNewUser, welcomeViewCompleted, mcpResponsesCollapsed, terminalOutputLineLimit, localClineRulesToggles, localWindsurfRulesToggles, localCursorRulesToggles, localWorkflowToggles, } = await getAllExtensionState(this.context);
