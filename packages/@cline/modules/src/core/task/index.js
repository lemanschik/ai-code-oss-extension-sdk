...
import { parseAssistantMessageV2, parseAssistantMessageV3 } from "../assistant-message";
...
import { sendRelinquishControlEvent } from "../controller/ui/subscribeToRelinquishControl";
...
import { ensureRulesDirectoryExists, ensureTaskDirectoryExists, getSavedApiConversationHistory, getSavedClineMessages, GlobalFileNames, } from "../storage/disk";
...
export class Task {
    ...
    messageStateHandler;
    constructor(context, mcpHub, workspaceTracker, updateTaskHistory, postStateToWebview, reinitExistingTaskFromId, cancelTask, apiConfiguration, autoApprovalSettings, browserSettings, preferredLanguage, openaiReasoningEffort, mode, strictPlanModeEnabled, shellIntegrationTimeout, terminalReuseEnabled, terminalOutputLineLimit, defaultTerminalProfile, enableCheckpointsSetting, cwd, cacheService, task, images, files, historyItem) {
        ...
        this.mcpHub.setNotificationCallback(async (serverName, level, message) => {
            ...
            onRetryAttempt: async (attempt, maxRetries, delay, error) => {
                ...
                if (lastApiReqStartedIndex !== -1) {
                    try {
                        ...
                        await this.postStateToWebview().catch((e) => console.error("Error posting state to webview in onRetryAttempt:", e));
                        console.log(`[Task ${this.taskId}] API Auto-Retry Status Update: Attempt ${attempt}/${maxRetries}, Delay: ${delay}ms`);
                    }
                    catch (e) {
        ...
        this.toolExecutor = new ToolExecutor(this.context, this.taskState, this.messageStateHandler, this.api, this.urlContentFetcher, this.browserSession, this.diffViewProvider, this.mcpHub, this.fileContextTracker, this.clineIgnoreController, this.workspaceTracker, this.contextManager, this.cacheService, this.autoApprovalSettings, this.browserSettings, cwd, this.taskId, this.mode, strictPlanModeEnabled, this.say.bind(this), this.ask.bind(this), this.saveCheckpoint.bind(this), this.sayAndCreateMissingParamError.bind(this), this.removeLastPartialMessageIfExistsWithType.bind(this), this.executeCommandTool.bind(this), this.doesLatestTaskCompletionHaveNewChanges.bind(this));
    }
    updateMode(mode) {
        this.mode = mode;
        ...
        switch (restoreType) {
            ...
            case "workspace":
                ...
                if (!this.checkpointTracker && !this.taskState.checkpointTrackerErrorMessage) {
                    try {
                        this.checkpointTracker = await CheckpointTracker.create(this.taskId, this.context.globalStorageUri.fsPath, this.enableCheckpoints);
    ...
    async presentMultifileDiff(messageTs, seeNewChangesSinceLastTaskCompletion) {
        ...
        try {
            if (seeNewChangesSinceLastTaskCompletion) {
                ...
                const lastTaskCompletedMessageCheckpointHash = findLast(this.messageStateHandler.getClineMessages().slice(0, messageIndex), (m) => m.say === "completion_result")?.lastCheckpointHash; // ask is only used to relinquish control, its the last say we care about
    ...
    async doesLatestTaskCompletionHaveNewChanges() {
        ...
        // Get last task completed
        const lastTaskCompletedMessage = findLast(this.messageStateHandler.getClineMessages().slice(0, messageIndex), (m) => m.say === "completion_result");
        try {
            // Get last task completed
    ...
    async resumeTaskFromHistory() {
        ...
        const lastRelevantMessageIndex = findLastIndex(savedClineMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"));
        ...
        const [taskResumptionMessage, userResponseMessage] = formatResponse.taskResumption(this.mode === "plan" ? "plan" : "act", agoText, this.cwd, wasRecent, responseText, hasPendingFileContextWarnings);
        ...
        if (!isAttemptCompletionMessage) {
            ...
            if (this.checkpointTracker) {
                ...
                this.checkpointTracker.commit().then(async (commitHash) => {
                    if (commitHash) {
                        const lastCheckpointMessageIndex = findLastIndex(this.messageStateHandler.getClineMessages(), (m) => m.say === "checkpoint_created");
            ...
            if (this.checkpointTracker &&
                ...
                const lastCompletionResultMessage = findLast(this.messageStateHandler.getClineMessages(), (m) => m.say === "completion_result" || m.ask === "completion_result");
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const [localCursorRulesFileInstructions, localCursorRulesDirInstructions] = await getLocalCursorRules(this.cwd, cursorLocalToggles);
        ...
        if (globalClineRulesFileInstructions ||
            ...
            const userInstructions = addUserInstructions(globalClineRulesFileInstructions, localClineRulesFileInstructions, localCursorRulesFileInstructions, localCursorRulesDirInstructions, localWindsurfRulesFileInstructions, clineIgnoreInstructions, preferredLanguageInstructions);
            systemPrompt += userInstructions;
        }
        const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(this.messageStateHandler.getApiConversationHistory(), this.messageStateHandler.getClineMessages(), this.api, this.taskState.conversationHistoryDeletedRange, previousApiReqIndex, await ensureTaskDirectoryExists(this.getContext(), this.taskId));
        ...
        try {
            ...
            if (isAnthropic && isAnthropicContextWindowError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
                this.taskState.conversationHistoryDeletedRange = this.contextManager.getNextTruncationRange(this.messageStateHandler.getApiConversationHistory(), this.taskState.conversationHistoryDeletedRange, "quarter");
                await this.messageStateHandler.saveClineMessagesAndUpdateHistory();
                ...
                if (isOpenRouterContextWindowError || isAnthropicContextWindowError) {
                    const truncatedConversationHistory = this.contextManager.getTruncatedMessages(this.messageStateHandler.getApiConversationHistory(), this.taskState.conversationHistoryDeletedRange);
                ...
                const lastApiReqStartedIndex = findLastIndex(this.messageStateHandler.getClineMessages(), (m) => m.say === "api_req_started");
                if (lastApiReqStartedIndex !== -1) {
                    const clineMessages = this.messageStateHandler.getClineMessages();
                    const currentApiReqInfo = JSON.parse(clineMessages[lastApiReqStartedIndex].text || "{}");
                ...
                const manualRetryApiReqIndex = findLastIndex(this.messageStateHandler.getClineMessages(), (m) => m.say === "api_req_started");
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        if (this.taskState.consecutiveMistakeCount >= 3) {
            ...
            const { response, text, images, files } = await this.ask("mistake_limit_reached", this.api.getModel().id.includes("claude")
        ...
        if (this.autoApprovalSettings.enabled &&
            ...
            const { response, text, images, files } = await this.ask("auto_approval_max_req_reached", `Cline has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`);
        ...
        if (isFirstRequest &&
            ...
            try {
                ...
                this.checkpointTracker = await pTimeout(CheckpointTracker.create(this.taskId, this.context.globalStorageUri.fsPath, this.enableCheckpoints), {
                    milliseconds: 15_000,
        ...
        if (clinerulesError === true) {
            await this.say("error", "Issue with processing the /newrule command. Double check that, if '.clinerules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.");
        }
            ...
            return await Promise.all(userContent.map(async (block) => {
                if (block.type === "text") {
                    ...
                    if (block.text.includes("<feedback>") ||
                        ...
                        const { processedText, needsClinerulesFileCheck: needsCheck } = await parseSlashCommands(parsedText, localWorkflowToggles, globalWorkflowToggles);
                        if (needsCheck) {
    ...
    async getEnvironmentDetails(includeFileDetails = false) {
        ...
        const visibleFilePaths = (await HostProvider.window.getVisibleTabs({})).paths.map((absolutePath) => path.relative(this.cwd, absolutePath));
        ...
        details += "\n\n# VSCode Open Tabs";
        const openTabPaths = (await HostProvider.window.getOpenTabs({})).paths.map((absolutePath) => path.relative(this.cwd, absolutePath));
        // Filter paths through clineIgnoreController
