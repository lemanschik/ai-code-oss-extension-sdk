import { showSystemNotification } from "../../integrations/notifications/index.js";
import { listFiles } from "../../services/glob/list-files.js";
import { telemetryService } from "../../services/posthog/telemetry/TelemetryService.js";
...
import { findLast, findLastIndex, parsePartialArrayString } from "../../shared/array.js";
import { createAndOpenGitHubIssue } from "../../utils/github-url-utils.js";
import { getReadablePath, isLocatedInWorkspace } from "../../utils/path.js";
import { processFilesIntoText } from "../../integrations/misc/extract-text.js";
import { BrowserSession } from "../../services/browser/BrowserSession.js";
import { browserActions, COMPLETION_RESULT_CHANGES_FLAG, } from "../../shared/ExtensionMessage.js";
import { extractFileContent } from "../../integrations/misc/extract-file-content.js";
import { COMMAND_REQ_APP_STRING } from "../../shared/combineCommandSequences.js";
import { fileExistsAtPath } from "../../utils/fs.js";
import { isClaude4ModelFamily, isGemini2dot5ModelFamily, isGrok4ModelFamily, modelDoesntSupportWebp } from "../../utils/model-utils.js";
...
import { USE_EXPERIMENTAL_CLAUDE4_FEATURES } from "./index.js";
import { constructNewFileContent } from "../assistant-message/diff.js";
import { StreamingJsonReplacer } from "../assistant-message/diff-json.js";
import { loadMcpDocumentation } from "../prompts/loadMcpDocumentation.js";
import { formatResponse } from "../prompts/responses.js";
import { ensureTaskDirectoryExists } from "../storage/disk.js";
import { AutoApprove } from "./tools/autoApprove.js";
import { showNotificationForApprovalIfAutoApprovalEnabled } from "./utils.js";
export class ToolExecutor {
    ...
    context, taskState, messageStateHandler, api, urlContentFetcher, browserSession, diffViewProvider, mcpHub, fileContextTracker, clineIgnoreController, workspaceTracker, contextManager, cacheService, 
    // Configuration & Settings
    autoApprovalSettings, browserSettings, cwd, taskId, mode, strictPlanModeEnabled, 
    // Callbacks to the Task (Entity)
    say, ask, saveCheckpoint, sayAndCreateMissingParamError, removeLastPartialMessageIfExistsWithType, executeCommandTool, doesLatestTaskCompletionHaveNewChanges) {
    ...
    isPlanModeToolRestricted(toolName) {
        const planModeRestrictedTools = ["write_to_file", "replace_in_file"];
    ...
    pushAdditionalToolFeedback = (feedback, images, fileContentString) => {
    ...
    askApproval = async (type, block, partialMessage) => {
    ...
    handleError = async (action, error, block) => {
    ...
    removeClosingTag = (block, tag, text) => {
        if (!block.partial) {
            return text || "";
        }
        if (!text) {
    ...
    async handleStreamingJsonReplacement(block, relPath, currentFullJson) {
        ...
        if (block.partial) {
            ...
            if (!this.taskState.streamingJsonReplacer) {
                ...
                // Set up callbacks
                const onContentUpdated = (newContent, _isFinalItem, changeLocation) => {
                ...
                this.taskState.streamingJsonReplacer = new StreamingJsonReplacer(this.diffViewProvider.originalContent || "", onContentUpdated, onError);
                this.taskState.lastProcessedJsonLength = 0;
            }
    ...
    async executeTool(block) {
        ...
        switch (block.name) {
                ...
                try {
                    ...
                    if (diff) {
                        ...
                        if (isNextGenModel && USE_EXPERIMENTAL_CLAUDE4_FEATURES && currentFullJson) {
                            ...
                            try {
                                ...
                                this.pushToolResult(formatResponse.toolError(`${error?.message}\n\n` +
                    ...
                    if (block.partial) {
                        ...
                        if (await this.shouldAutoApproveToolWithPath(block.name, relPath)) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "tool");
                        ...
                        if (userEdits) {
                            ...
                            this.pushToolResult(formatResponse.fileEditWithUserChanges(relPath, userEdits, autoFormattingEdits, finalContent, newProblemsMessage), block);
                        }
                        else {
                ...
                try {
                    if (block.partial) {
                        ...
                        if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to read ${path.basename(absolutePath)}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "tool");
                ...
                try {
                    if (block.partial) {
                        ...
                        if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to view directory ${path.basename(absolutePath)}/`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "tool");
                            const didApprove = await this.askApproval("tool", block, completeMessage);
                            if (!didApprove) {
                                telemetryService.captureToolUsage(this.taskId, block.name, this.api.getModel().id, false, false);
            ...
            case "list_code_definition_names": {
                ...
                try {
                    if (block.partial) {
                        ...
                        if (!relDirPath) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("list_code_definition_names", "path"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        this.taskState.consecutiveMistakeCount = 0;
                        ...
                        if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to view source code definitions in ${path.basename(absolutePath)}/`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "tool");
                            const didApprove = await this.askApproval("tool", block, completeMessage);
                            if (!didApprove) {
                                telemetryService.captureToolUsage(this.taskId, block.name, this.api.getModel().id, false, false);
            ...
            case "search_files": {
                ...
                try {
                    if (block.partial) {
                        ...
                        this.taskState.consecutiveMistakeCount = 0;
                        const absolutePath = path.resolve(this.cwd, relDirPath);
                        const results = await regexSearchFiles(this.cwd, absolutePath, regex, filePattern, this.clineIgnoreController);
                        const completeMessage = JSON.stringify({
                            ...sharedMessageProps,
                            content: results,
                            operationIsLocatedInWorkspace: await isLocatedInWorkspace(block.params.path),
                        ...
                        if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to search files in ${path.basename(absolutePath)}/`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "tool");
                            const didApprove = await this.askApproval("tool", block, completeMessage);
                            if (!didApprove) {
                                telemetryService.captureToolUsage(this.taskId, block.name, this.api.getModel().id, false, false);
                    ...
                    if (block.partial) {
                        if (action === "launch") {
                            if (this.shouldAutoApproveTool(block.name)) {
                                ...
                                await this.say("browser_action_launch", this.removeClosingTag(block, "url", url), undefined, undefined, block.partial);
                            }
                            else {
                                this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch");
                                await this.ask("browser_action_launch", this.removeClosingTag(block, "url", url), block.partial).catch(() => { });
                            }
                        }
                        else {
                        ...
                        if (action === "launch") {
                            ...
                            if (this.shouldAutoApproveTool(block.name)) {
                                ...
                                showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to use a browser and launch ${url}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                                this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch");
                            ...
                            if (action === "click") {
                                if (!coordinate) {
                                    this.taskState.consecutiveMistakeCount++;
                                    this.pushToolResult(await this.sayAndCreateMissingParamError("browser_action", "coordinate"), block);
                                    await this.browserSession.closeBrowser();
                                    await this.saveCheckpoint();
                        ...
                        switch (action) {
                            ...
                            case "scroll_up":
                                ...
                                this.pushToolResult(formatResponse.toolResult(`The browser action has been executed. The console logs and screenshot have been captured for your analysis.\n\nConsole logs:\n${browserActionResult.logs || "(No new logs)"}\n\n(REMEMBER: if you need to proceed to using non-\`browser_action\` tools or launch a new browser, you MUST first close this browser. For example, if after analyzing the logs and screenshot you need to edit a file, you must first close the browser before you can use the write_to_file tool.)`, browserActionResult.screenshot ? [browserActionResult.screenshot] : []), block);
                                await this.saveCheckpoint();
                                break;
                            case "close":
                                this.pushToolResult(formatResponse.toolResult(`The browser has been closed. You may now proceed to using other tools.`), block);
                                await this.saveCheckpoint();
                                break;
                        }
                        break;
                ...
                try {
                    if (block.partial) {
                        ...
                        if (!requiresApprovalRaw) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("execute_command", "requires_approval"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        this.taskState.consecutiveMistakeCount = 0;
                        ...
                        if (ignoredFileAttemptedToAccess) {
                            ...
                            this.pushToolResult(formatResponse.toolError(formatResponse.clineIgnoreError(ignoredFileAttemptedToAccess)), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        ...
                        if ((!requiresApprovalPerLLM && autoApproveSafe) ||
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to execute a command: ${command}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                    ...
                    if (block.partial) {
                        ...
                        if (mcp_arguments) {
                            try {
                                ...
                                this.pushToolResult(formatResponse.toolError(formatResponse.invalidMcpToolArgumentError(server_name, tool_name)), block);
                                await this.saveCheckpoint();
                                break;
                            }
                        }
                        ...
                        if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to use ${tool_name} on ${server_name}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server");
                        ...
                        this.pushToolResult(formatResponse.toolResult(toolResultText, supportsImages ? toolResultImages : undefined), block);
                        await this.saveCheckpoint();
                        break;
                    }
                }
            ...
            case "access_mcp_resource": {
                ...
                try {
                    if (block.partial) {
                        ...
                        if (!server_name) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("access_mcp_resource", "server_name"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        if (!uri) {
                        ...
                        if (this.shouldAutoApproveTool(block.name)) {
                            ...
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to access ${uri} on ${server_name}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server");
                            const didApprove = await this.askApproval("use_mcp_server", block, completeMessage);
                            if (!didApprove) {
                                await this.saveCheckpoint();
            ...
            case "ask_followup_question": {
                ...
                try {
                    if (block.partial) {
                        ...
                        if (!question) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("ask_followup_question", "question"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        this.taskState.consecutiveMistakeCount = 0;
                        ...
                        const { text, images, files: followupFiles, } = await this.ask("followup", JSON.stringify(sharedMessage), false);
                        // Check if options contains the text response
                        if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
                            // Valid option selected, don't show user message in UI
                            // Update last followup message with selected option
                            const lastFollowupMessage = findLast(this.messageStateHandler.getClineMessages(), (m) => m.ask === "followup");
                        ...
                        this.pushToolResult(formatResponse.toolResult(`<answer>\n${text}\n</answer>`, images, fileContentString), block);
                        await this.saveCheckpoint();
                        break;
                    }
                }
                ...
                try {
                    if (block.partial) {
                        ...
                        if (text || (images && images.length > 0) || (newTaskFiles && newTaskFiles.length > 0)) {
                            ...
                            this.pushToolResult(formatResponse.toolResult(`The user provided feedback instead of creating a new task:\n<feedback>\n${text}\n</feedback>`, images, fileContentString), block);
                        }
                        else {
                            // If no response, the user clicked the "Create New Task" button
                            this.pushToolResult(formatResponse.toolResult(`The user has created a new task with the provided context.`), block);
                        }
                        await this.saveCheckpoint();
                        break;
                    }
                ...
                try {
                    if (block.partial) {
                        ...
                        if (text || (images && images.length > 0) || (condenseFiles && condenseFiles.length > 0)) {
                            ...
                            this.pushToolResult(formatResponse.toolResult(`The user provided feedback on the condensed conversation summary:\n<feedback>\n${text}\n</feedback>`, images, fileContentString), block);
                        }
                        else {
                            ...
                            this.taskState.conversationHistoryDeletedRange = this.contextManager.getNextTruncationRange(apiConversationHistory, this.taskState.conversationHistoryDeletedRange, keepStrategy);
                            await this.messageStateHandler.saveClineMessagesAndUpdateHistory();
                            await this.contextManager.triggerApplyStandardContextTruncationNoticeChange(Date.now(), await ensureTaskDirectoryExists(this.context, this.taskId));
                        }
                        await this.saveCheckpoint();
                        break;
                    }
            ...
            case "report_bug": {
                ...
                try {
                    if (block.partial) {
                        await this.ask("report_bug", JSON.stringify({
                        ...
                        if (!steps_to_reproduce) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("report_bug", "steps_to_reproduce"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        if (!api_request_output) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("report_bug", "api_request_output"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        if (!additional_context) {
                            this.taskState.consecutiveMistakeCount++;
                            this.pushToolResult(await this.sayAndCreateMissingParamError("report_bug", "additional_context"), block);
                            await this.saveCheckpoint();
                            break;
                        }
                        this.taskState.consecutiveMistakeCount = 0;
                        ...
                        if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
                            ...
                            this.pushToolResult(formatResponse.toolResult(`The user did not submit the bug, and provided feedback on the Github issue generated instead:\n<feedback>\n${text}\n</feedback>`, images, fileContentString), block);
                        }
                        else {
                            // If no response, the user accepted the condensed version
                            this.pushToolResult(formatResponse.toolResult(`The user accepted the creation of the Github issue.`), block);
                            try {
            ...
            case "web_fetch": {
                ...
                try {
                    if (block.partial) {
                        ...
                        if (this.shouldAutoApproveTool("web_fetch")) {
                            this.removeLastPartialMessageIfExistsWithType("ask", "tool");
                            await this.say("tool", partialMessage, undefined, undefined, block.partial);
                        }
                        else {
                        ...
                        if (this.shouldAutoApproveTool("web_fetch")) {
                            ...
                            await this.say("tool", completeMessage, undefined, undefined, false);
                            this.taskState.consecutiveAutoApprovedRequestsCount++;
                            telemetryService.captureToolUsage(this.taskId, "web_fetch", this.api.getModel().id, true, true);
                        }
                        else {
                            showNotificationForApprovalIfAutoApprovalEnabled(`Cline wants to fetch content from ${url}`, this.autoApprovalSettings.enabled, this.autoApprovalSettings.enableNotifications);
                            this.removeLastPartialMessageIfExistsWithType("say", "tool");
                            const didApprove = await this.askApproval("tool", block, completeMessage);
                            if (!didApprove) {
                                telemetryService.captureToolUsage(this.taskId, "web_fetch", this.api.getModel().id, false, false);
                                await this.saveCheckpoint();
                                break;
                            }
                            telemetryService.captureToolUsage(this.taskId, "web_fetch", this.api.getModel().id, false, true);
                        }
            ...
            case "plan_mode_respond": {
                ...
                try {
                    if (block.partial) {
                        ...
                        let { text, images, files: planResponseFiles, } = await this.ask("plan_mode_respond", JSON.stringify(sharedMessage), false);
                        ...
                        if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
                            // Valid option selected, don't show user message in UI
                            // Update last followup message with selected option
                            const lastPlanMessage = findLast(this.messageStateHandler.getClineMessages(), (m) => m.ask === "plan_mode_respond");
                            if (lastPlanMessage) {
                        ...
                        if (this.taskState.didRespondToPlanAskBySwitchingMode) {
                            this.pushToolResult(formatResponse.toolResult(`[The user has switched to ACT MODE, so you may now proceed with the task.]` +
                                (text
                                    ? `\n\nThe user also provided the following message when switching to ACT MODE:\n<user_message>\n${text}\n</user_message>`
                                    : ""), images, fileContentString), block);
                        }
                        else {
                            // if we didn't switch to ACT MODE, then we can just send the user_feedback message
                            this.pushToolResult(formatResponse.toolResult(`<user_message>\n${text}\n</user_message>`, images, fileContentString), block);
                        }
                        //
                        break;
                    }
            ...
            case "attempt_completion": {
                ...
                const addNewChangesFlagToLastCompletionResultMessage = async () => {
                    ...
                    const lastCompletionResultMessage = lastCompletionResultMessageIndex !== -1 ? clineMessages[lastCompletionResultMessageIndex] : undefined;
                    ...
                    if (block.partial) {
                        if (command) {
                            ...
                            if (lastMessage && lastMessage.ask === "command") {
                                ...
                                await this.say("completion_result", this.removeClosingTag(block, "result", result), undefined, undefined, false);
                            ...
                            await this.say("completion_result", this.removeClosingTag(block, "result", result), undefined, undefined, block.partial);
                        }
                        break;
                    }
                    else {
