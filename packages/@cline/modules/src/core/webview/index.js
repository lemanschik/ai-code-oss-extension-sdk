...
export class WebviewProvider {
    ...
    static getSidebarInstance() {
        return Array.from(this.activeInstances).find((instance) => instance.getWebview() && "onDidChangeVisibility" in instance.getWebview());
    }
    static getTabInstances() {
        return Array.from(this.activeInstances).filter((instance) => instance.getWebview() && "onDidChangeViewState" in instance.getWebview());
    }
    ...
    getDevServerPort() {
        ...
        return readFile(portFilePath, "utf8")
            ...
            .catch((err) => {
            console.warn(`[getDevServerPort] Port file not found or couldn't be read at ${portFilePath}, using default port: ${DEFAULT_PORT}`);
            return DEFAULT_PORT;
        });
    }
    /**
        ...
        try {
            ...
            if (process.env.IS_DEV) {
                HostProvider.window.showMessage({
                    ...
                    message: "Cline: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
                });
            }
