...
export class SapAiCoreHandler {
    ...
    async *createMessage(systemPrompt, messages) {
        ...
        if (anthropicModels.includes(model.id)) {
            ...
            if (model.id === "anthropic--claude-4-sonnet" ||
        ...
        try {
            ...
            else if (model.id === "anthropic--claude-4-sonnet" ||
                model.id === "anthropic--claude-4-opus" ||
                model.id === "anthropic--claude-3.7-sonnet") {
    ...
    async *streamCompletion(stream, model) {
    ...
    async *streamCompletionSonnet37(stream, model) {
    ...
    async *streamCompletionGPT(stream, model) {
        let currentContent = "";
        let inputTokens = 0;
        let outputTokens = 0;
        try {
    ...
    async *streamCompletionGemini(stream, model) {
        let promptTokens = 0;
        let outputTokens = 0;
    ...
    convertToGeminiFormat(systemPrompt, messages) {
