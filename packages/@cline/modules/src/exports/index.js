...
export function createClineAPI(sidebarController) {
    const api = {
        startNewTask: async (task, images) => {
            ...
            HostProvider.get().logToChannel(`Task started with message: ${task ? `"${task}"` : "undefined"} and ${images?.length || 0} image(s)`);
        },
        sendMessage: async (message, images) => {
            HostProvider.get().logToChannel(`Sending message: ${message ? `"${message}"` : "undefined"} with ${images?.length || 0} image(s)`);
