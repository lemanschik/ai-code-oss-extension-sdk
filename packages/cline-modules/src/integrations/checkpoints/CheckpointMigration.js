...
export async function cleanupLegacyCheckpoints(globalStoragePath) {
    try {
        ...
        const folderStats = await Promise.all(taskFolders.map(async (folder) => {
                ...
                for (const folder of folderStats) {
                    ...
                    if (await fileExistsAtPath(folderCheckpointsDir)) {
                        ...
                        try {
                            ...
                            HostProvider.get().logToChannel(`Warning: Failed to delete checkpoints in ${folder.folder}, continuing...`);
                        }
                    }
                }
