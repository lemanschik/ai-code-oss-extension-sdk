...
let commitGenerationAbortController = undefined;
...
Commit message:`;
        ...
        const systemPrompt = "You are a helpful assistant that generates concise and descriptive git commit messages based on git diffs.";
 ...
 */
function formatGitDiffPrompt(gitDiff) {
 ...
 */
export async function copyCommitMessageToClipboard(message) {
 ...
 */
export async function showCommitMessageOptions(message) {
    const copyAction = "Copy to Clipboard";
    const applyAction = "Apply to Git Input";
    const editAction = "Edit Message";
    const selectedAction = (await HostProvider.window.showMessage({
        type: ShowMessageType.INFORMATION,
 ...
 */
async function applyCommitMessageToGitInput(message) {
 ...
 */
async function editCommitMessage(message) {
    const document = await vscode.workspace.openTextDocument({
        content: message,
        language: "markdown",
    });
    await HostProvider.window.showTextDocument(ShowTextDocumentRequest.create({
