    ...
    try {
        ...
        if (err instanceof Error) {
            if (err.message.includes("ENOENT")) {
                throw new Error(`Failed to find the Claude Code executable.
Make sure it's installed and available in your PATH or properly set in your provider settings.`, { cause: err });
            }
            if (err.message.includes("E2BIG")) {
                throw new Error(`Executing Claude Code failed due to a long system prompt. The maximum argument length is 131072 bytes. 
...
`, { cause: err });
            ...
            if (err.message.includes("ENAMETOOLONG")) {
                throw new Error(`Executing Claude Code failed due to a long system prompt. Windows has a limit of 8191 characters, which makes the integration with Cline not work properly.
