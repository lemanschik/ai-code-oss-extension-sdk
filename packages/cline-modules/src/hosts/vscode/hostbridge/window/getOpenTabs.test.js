...
describe("Hostbridge - Window - getOpenTabs", () => {
    ...
    it("should return empty array when no tabs are open", async () => {
        ...
        assert.strictEqual(response.paths.length, 0, `Should return empty array when no tabs are open. Found: ${JSON.stringify(response.paths)}`);
    });
    it("should return paths of open text document tabs", async () => {
        ...
        assert.strictEqual(response.paths.length, 2, `Expected 2 tabs, got ${response.paths.length}. Found tabs: ${JSON.stringify(response.paths)}`);
    });
    it("should return all open tabs even when multiple files are opened in the same ViewColumn", async () => {
        ...
        assert.strictEqual(response.paths.length, 3, `Expected 3 open tabs, got ${response.paths.length}. Found: ${JSON.stringify(response.paths)}`);
    });
});
