...
describe("Hostbridge - Window - getVisibleTabs", () => {
    ...
    it("should return empty array when no visible editors are open", async () => {
        ...
        assert.strictEqual(response.paths.length, 0, `Should return empty array when no visible editors are open. Found tabs: ${JSON.stringify(response.paths)}`);
    });
    it("should return paths of visible text editors", async () => {
        ...
        assert.strictEqual(response.paths.length, 1, `Expected 1 visible editor, got ${response.paths.length}. Found: ${JSON.stringify(response.paths)}`);
        ...
        assert.strictEqual(response2.paths.length, 2, `Expected 2 visible editors, got ${response2.paths.length}. Found: ${JSON.stringify(response2.paths)}`);
    });
    it("should only return visible editors, not all open tabs", async () => {
        ...
        assert.strictEqual(response.paths.length, actualVisibleEditors, `Response should match actual visible editors count: ${actualVisibleEditors}`);
    });
    it("should return only visible editors from multiple columns with multiple files", async () => {
        ...
        assert.strictEqual(response.paths.length, 2, `Expected 2 visible editors, got ${response.paths.length}. Found: ${JSON.stringify(response.paths)}`);
        // Verify that we have the correct number of visible text editors
        const actualVisibleEditors = vscode.window.visibleTextEditors.length;
        assert.strictEqual(response.paths.length, actualVisibleEditors, `Response should match actual visible editors count: ${actualVisibleEditors}`);
    });
