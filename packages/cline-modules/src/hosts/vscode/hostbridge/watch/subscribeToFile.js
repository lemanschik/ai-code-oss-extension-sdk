...
import { getRequestRegistry } from "../../hostbridge-grpc-handler";
...
export async function subscribeToFile(request, responseStream, requestId) {
    ...
    try {
                ...
                if (eventType === "change") {
                    try {
                        ...
                        if (watcherInfo) {
                            ...
                            if (now - lastTime < DEBOUNCE_DELAY) {
                                console.log(`[DEBUG] Debouncing change event for ${filePath} (${now - lastTime}ms since last event)`);
                    ...
                    try {
                        ...
                        if (watcherInfo) {
                            ...
                            const lastTime = watcherInfo.lastEventTime.get(eventType) || 0;
                            if (now - lastTime < DEBOUNCE_DELAY) {
                                console.log(`[DEBUG] Debouncing creation event for ${filePath} (${now - lastTime}ms since last event)`);
                                return; // Skip this event due to debounce
                            }
                            // Update the last event time
                            watcherInfo.lastEventTime.set(eventType, now);
                        ...
                        if (watcherInfo) {
                            ...
                            const lastTime = watcherInfo.lastEventTime.get(eventType) || 0;
                            if (now - lastTime < DEBOUNCE_DELAY) {
                                console.log(`[DEBUG] Debouncing deletion event for ${filePath} (${now - lastTime}ms since last event)`);
                                return; // Skip this event due to debounce
                            }
                            // Update the last event time
                            watcherInfo.lastEventTime.set(eventType, now);
        ...
        if (requestId) {
            getRequestRegistry().registerRequest(requestId, cleanup, { type: "file_subscription", path: filePath }, responseStream);
        }
    }
    catch (error) {
