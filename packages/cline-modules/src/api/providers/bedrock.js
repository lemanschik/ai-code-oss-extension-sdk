...
export class AwsBedrockHandler {
    ...
    async *createMessage(systemPrompt, messages) {
        ...
        const baseModelId = (this.options.awsBedrockCustomSelected ? this.options.awsBedrockCustomModelBaseId : modelId) || modelId;
    ...
    async getAwsCredentials() {
        ...
        const useProfile = (this.options.awsAuthentication === undefined && this.options.awsUseProfile) ||
            ...
            if (response.stream) {
                ...
                for await (const chunk of response.stream) {
                    ...
                    if (chunk.contentBlockStart) {
                        ...
                        if (blockStart.start?.type === "thinking" ||
    ...
    *handleBedrockStreamError(chunk) {
    ...
    getInferenceConfig(modelInfo, modelType) {
        ...
        if (modelType === "anthropic") {
            ...
            const baseModelId = (this.options.awsBedrockCustomSelected ? this.options.awsBedrockCustomModelBaseId : this.getModel().id) ||
                this.getModel().id;
    ...
    shouldEnableReasoning(baseModelId, budgetTokens) {
        return ((baseModelId.includes("3-7") || baseModelId.includes("sonnet-4") || baseModelId.includes("opus-4")) &&
            budgetTokens !== 0);
    }
    /**
     ...
     */
    async *createAnthropicMessage(systemPrompt, messages, modelId, model) {
    ...
    async *createNovaMessage(systemPrompt, messages, modelId, model) {
        ...
        const messagesWithCache = this.options.awsBedrockUsePromptCache && model.info.supportsPromptCache
            ? this.applyCacheControlToMessages(formattedMessages, lastUserMsgIndex, secondLastMsgUserIndex)
            : formattedMessages;
