 ...
 */
export function toolDefinitionToAntmlDefinition(toolDef) {
    ...
    propEntries.forEach(([propName, propDef], index) => {
        rawJson += `"${propName}": {`;
        rawJson += `"description": "${propDef.description || ""}", `;
        rawJson += `"type": "${propDef.type || "string"}"`;
        rawJson += `}`;
 ...
 */
export function toolDefinitionsToAntmlDefinitions(toolDefs) {
 ...
 */
export function toolDefinitionToAntmlCallExample(toolDef, exampleValues = {}) {
 ...
 */
export function createAntmlToolPrompt(toolDefs, includeInstructions = true, systemPrompt = "") {
 ...
 */
export function toolDefinitionToSimpleXml(toolDef) {
    ...
    if (Object.keys(properties).length > 0) {
        ...
        for (const [paramName, paramDef] of Object.entries(properties)) {
            ...
            const paramDescription = paramDef.description || "No description.";
 ...
 */
export function toolDefinitionsToSimpleXml(toolDefs) {
 ...
 */
export function createSimpleXmlToolPrompt(toolDefs, includeInstructions = true) {
    if (toolDefs.length === 0) {
        return "";
    }
    let prompt = "";
