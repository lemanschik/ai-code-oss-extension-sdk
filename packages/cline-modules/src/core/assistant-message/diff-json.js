...
export class StreamingJsonReplacer {
    ...
    onErrorCallback;
    itemsProcessed = 0;
    successfullyParsedItems = [];
    constructor(initialContent, onContentUpdatedCallback, onErrorCallback) {
        ...
        this.parser.onValue = (parsedElementInfo) => {
            ...
            if (value && typeof value === "object" && "old_string" in value && "new_string" in value) {
                const item = value; // Value here is confirmed to be an object
    ...
    getSuccessfullyParsedItems() {
        return [...this.successfullyParsedItems]; // Return a copy
    }
    calculateChangeLocation(oldStr, newStr) {
