 ...
 */
export function parseAssistantMessageV1(assistantMessage) {
    const contentBlocks = [];
    ...
    for (let i = 0; i < assistantMessage.length; i++) {
        ...
        for (const toolUseOpeningTag of possibleToolUseOpeningTags) {
            if (accumulator.endsWith(toolUseOpeningTag)) {
                ...
                const toolName = toolUseOpeningTag.slice(1, -1);
                currentToolUse = {
                    type: "tool_use",
                    name: toolName,
                    params: {},
        ...
        if (currentToolUse && currentParamName) {
            ...
            if (currentCharIndex >= closeTag.length - 1 &&
                ...
                const value = assistantMessage
                    .slice(currentParamValueStart, // Start after the opening tag
...
export function parseAssistantMessageV3(assistantMessage) {
    const contentBlocks = [];
    let currentTextContentStart = 0; // Index where the current text block started
    let currentTextContent = undefined;
    let currentToolUseStart = 0; // Index *after* the opening tag of the current tool use
