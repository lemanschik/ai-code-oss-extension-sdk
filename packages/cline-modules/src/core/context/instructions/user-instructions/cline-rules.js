...
import { synchronizeRuleToggles, getRuleFilesTotalContent } from "./rule-helpers";
        ...
        if (await isDirectory(globalClineRulesFilePath)) {
            try {
                ...
                if (rulesFilesTotalContent) {
                    const clineRulesFileInstructions = formatResponse.clineRulesGlobalDirectoryInstructions(globalClineRulesFilePath, rulesFilesTotalContent);
...
export async function refreshClineRulesToggles(context, workingDirectory) {
    // Global toggles
    const globalClineRulesToggles = (await getGlobalState(context, "globalClineRulesToggles")) || {};
