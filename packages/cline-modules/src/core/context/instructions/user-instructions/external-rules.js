...
import { getWorkspaceState, updateWorkspaceState } from "../../../storage/state";
import { synchronizeRuleToggles, combineRuleToggles, getRuleFilesTotalContent, readDirectoryRecursive, } from "./rule-helpers";
/**
 * Refreshes the toggles for windsurf and cursor rules
 */
export async function refreshExternalRulesToggles(context, workingDirectory) {
    // local windsurf toggles
    const localWindsurfRulesToggles = (await getWorkspaceState(context, "localWindsurfRulesToggles")) || {};
    ...
    const localCursorRulesToggles = (await getWorkspaceState(context, "localCursorRulesToggles")) || {};
