...
import { ensureRulesDirectoryExists, ensureWorkflowsDirectoryExists, GlobalFileNames } from "../../../storage/disk";
 ...
 */
export async function deleteRuleFile(context, rulePath, isGlobal, type) {
    try {
        ...
        if (isGlobal) {
            if (type === "workflow") {
                const toggles = (await getGlobalState(context, "globalWorkflowToggles")) || {};
                delete toggles[rulePath];
                await updateGlobalState(context, "globalWorkflowToggles", toggles);
            }
            else {
                const toggles = (await getGlobalState(context, "globalClineRulesToggles")) || {};
                delete toggles[rulePath];
            ...
            if (type === "workflow") {
                const toggles = (await getWorkspaceState(context, "workflowToggles")) || {};
                delete toggles[rulePath];
                await updateWorkspaceState(context, "workflowToggles", toggles);
            }
            else if (type === "cursor") {
                const toggles = (await getWorkspaceState(context, "localCursorRulesToggles")) || {};
                delete toggles[rulePath];
                await updateWorkspaceState(context, "localCursorRulesToggles", toggles);
            }
            else if (type === "windsurf") {
                const toggles = (await getWorkspaceState(context, "localWindsurfRulesToggles")) || {};
                delete toggles[rulePath];
                await updateWorkspaceState(context, "localWindsurfRulesToggles", toggles);
            }
            else {
                const toggles = (await getWorkspaceState(context, "localClineRulesToggles")) || {};
                delete toggles[rulePath];
