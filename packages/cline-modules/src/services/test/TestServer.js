 ...
 */
function createToolCallTracker(webviewProvider) {
    const tracker = {
        toolCalls: {},
        toolFailures: {},
    };
 ...
 */
async function updateAutoApprovalSettings(context, provider) {
    try {
 ...
 */
export function createTestServer(webviewProvider) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            if (typeof taskId === "string") {
                                apiConversationHistory = await getSavedApiConversationHistory(visibleWebview.controller.context, taskId);
                            }
                        }
                        catch (error) {
 ...
 */
export function createMessageCatcher(webviewProvider) {
 ...
 */
async function autoRespondToAsk(webviewProvider, askType, askText) {
    ...
    try {
        await askResponse(webviewProvider.controller, AskResponseRequest.create({
            responseType,
            text: responseText,
