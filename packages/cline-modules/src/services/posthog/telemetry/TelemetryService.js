...
class TelemetryService {
        ...
        if (globalTelemetryEnabled) {
            ...
            if (didUserOptIn) {
                void HostProvider.window
                    ...
                    message: "Anonymous Cline error and usage reporting is enabled, but VSCode telemetry is disabled. To enable error and usage reporting for this extension, enable VSCode telemetry in settings.",
                    options: {
                        items: ["Open Settings"],
                    },
                })
    ...
    captureExtensionActivated(installId) {
     ...
     */
    captureConversationTurnEvent(taskId, provider = "unknown", model = "unknown", source, tokenUsage = {}) {
     ...
     */
    captureTaskFeedback(taskId, feedbackType) {
     ...
     */
    captureToolUsage(taskId, tool, modelId, autoApproved, success) {
        this.capture({
     ...
     */
    captureDiffEditFailure(taskId, modelId, errorType) {
        this.capture({
     ...
     */
    captureBrowserToolStart(taskId, browserSettings) {
     ...
     */
    captureBrowserToolEnd(taskId, stats) {
        if (!this.isCategoryEnabled("browser")) {
            return;
        }
        this.capture({
     ...
     */
    captureBrowserError(taskId, errorType, errorMessage, context) {
        if (!this.isCategoryEnabled("browser")) {
            return;
        }
        this.capture({
     ...
     */
    captureOptionSelected(taskId, qty, mode) {
        this.capture({
     ...
     */
    captureOptionsIgnored(taskId, qty, mode) {
        this.capture({
     ...
     */
    captureGeminiApiPerformance(taskId, modelId, data) {
        this.capture({
